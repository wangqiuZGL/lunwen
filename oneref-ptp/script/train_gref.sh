...
echo "train_syntax_guided_attention_gref.sh"
...
# ========== 句法引导注意力参数 ==========
ENABLE_SYNTAX_BIAS=flase  # 是否启用句法引导注意力
...
######## 1. Warmup 训练（RefCOCOg / gref_umd）—— 提高 batch_size 提高显存利用
echo "=== 阶段1: Warmup 训练 ==="
CUDA_VISIBLE_DEVICES=0,1 python -m torch.distributed.launch \
  ...
  --dataset gref_umd --use_regress_box --frozen_backbone \
  ...
  --output_dir ${OUTPUT_DIR}/syntax_guided/gref_warmup \
  ${SYNTAX_ARGS}

######## 2. Finetune 训练（RefCOCOg / gref_umd）—— 使用句法引导注意力
...
CUDA_VISIBLE_DEVICES=0,1 python -m torch.distributed.launch \
  ...
  --sentencepiece_model ${BEIT3_CHECKPOINT}/beit3.spm \
  --finetune ${OUTPUT_DIR}/syntax_guided/gref_warmup/best_checkpoint.pth \
  --data_root ${DATA_ROOT} --split_root ${SPLIT_ROOT}/single_dataset \
  --output_dir ${OUTPUT_DIR}/syntax_guided/gref_finetune \
  ${SYNTAX_ARGS}

######## 3. 评估（RefCOCOg / gref_umd / val）
echo "=== 阶段3: 模型评估 ==="
CUDA_VISIBLE_DEVICES=0,1 python -m torch.distributed.launch \
...
echo "RefCOCOg数据集训练和评估完成！结果保存在 ${OUTPUT_DIR}/syntax_guided"
