...
echo "train_syntax_guided_attention_recoco+.sh"
...
CUDA_VISIBLE_DEVICES=0,1 python -m torch.distributed.launch \
  ...
  --output_dir ${OUTPUT_DIR}/syntax_guided/recoco+_warmup  \
  ${SYNTAX_ARGS}

######## 2. Finetune 训练（RefCOCO+ / unc+）—— 使用句法引导注意力
...
CUDA_VISIBLE_DEVICES=0,1 python -m torch.distributed.launch \
  ...
  --sentencepiece_model ${BEIT3_CHECKPOINT}/beit3.spm \
  --finetune ${OUTPUT_DIR}/syntax_guided/recoco+_warmup/best_checkpoint.pth \
  --data_root ${DATA_ROOT} --split_root ${SPLIT_ROOT}/single_dataset \
  --output_dir ${OUTPUT_DIR}/syntax_guided/recoco+_finetune \
  ${SYNTAX_ARGS}
...
CUDA_VISIBLE_DEVICES=0,1 python -m torch.distributed.launch \
  ...
  --sentencepiece_model ${BEIT3_CHECKPOINT}/beit3.spm \
  --data_root ${DATA_ROOT} --split_root ${SPLIT_ROOT}/single_dataset \
  --eval_model ${OUTPUT_DIR}/syntax_guided/recoco+_finetune/best_checkpoint.pth \
  --output_dir ${OUTPUT_DIR}/syntax_guided/recoco+_eval  \
  ${SYNTAX_ARGS}
...
echo "RefCOCO+数据集训练和评估完成！结果保存在 ${OUTPUT_DIR}/syntax_guided"
