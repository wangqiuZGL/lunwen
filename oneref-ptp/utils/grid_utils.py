...
def compute_grid_id(bbox_xywh, grid_size=16):
    ...
    Args:
        bbox_xywh: 归一化的bbox，格式为 [cx, cy, w, h] (0-1范围)
    ...
    if len(bbox_xywh) != 4:
        raise ValueError("bbox must use normalized [cx, cy, w, h] format")
...
def compute_grid_id_batch(bboxes_xywh, grid_size=16):
    ...
    Args:
        bboxes_xywh: (B, 4) 归一化的bboxes，格式为 [cx, cy, w, h]
        grid_size: 网格大小
...
def test_grid_id_computation():
    ...
    bbox1 = np.array([0.5, 0.5, 0.2, 0.2])
    grid_id1 = compute_grid_id(bbox1, grid_size=16)
    expected1 = 8 * 16 + 8  # 第8行第8列 = 136
    print(f"中心bbox [0.5, 0.5, 0.2, 0.2] -> Grid ID: {grid_id1} (期望: {expected1})")
    
    # 测试用例2: 左上角 (0.1, 0.1)
    bbox2 = np.array([0.1, 0.1, 0.1, 0.1])
    grid_id2 = compute_grid_id(bbox2, grid_size=16)
    expected2 = 1 * 16 + 1  # 第1行第1列 = 17
    print(f"左上角bbox [0.1, 0.1, 0.1, 0.1] -> Grid ID: {grid_id2} (期望: {expected2})")
    
    # 测试用例3: 右下角 (0.9, 0.9)
    bbox3 = np.array([0.9, 0.9, 0.1, 0.1])
    grid_id3 = compute_grid_id(bbox3, grid_size=16)
    expected3 = 14 * 16 + 14  # 第14行第14列 = 238
    print(f"右下角bbox [0.9, 0.9, 0.1, 0.1] -> Grid ID: {grid_id3} (期望: {expected3})")
    
    # 测试用例4: 边界情况 (cx=1.0)
    bbox4 = np.array([1.0, 1.0, 0.1, 0.1])
    grid_id4 = compute_grid_id(bbox4, grid_size=16)
    expected4 = 15 * 16 + 15  # 第15行第15列 = 255
    print(f"边界bbox [1.0, 1.0, 0.1, 0.1] -> Grid ID: {grid_id4} (期望: {expected4})")
