...
def one_ref_loss(args, batch_pred, batch_target, tgt_mask, contrastive_loss, visu_sim=None, seg_mask=None,
                 ...
                 ptp_grid_logits=None, ptp_grid_labels=None):
    ...
    if args.enable_ptp_aux and ptp_grid_logits is not None and ptp_grid_labels is not None:
        ...
        losses['loss_ptp'] = ptp_loss * ptp_loss_weight
        
        # 可选：计算Grid分类准确率（用于监控）
        with torch.no_grad():
            ptp_pred = torch.max(ptp_grid_logits, dim=1)[1]
            ptp_acc = (ptp_pred == ptp_grid_labels).float().mean()
