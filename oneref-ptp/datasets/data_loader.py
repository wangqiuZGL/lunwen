...
class TransVGDataset(data.Dataset):
    ...
    def pull_item(self, idx):
        if self.dataset == 'flickr':  # flickr
            ...
            # sed for segmentation and alignment
        ...
        if dataset in ["unc", "unc+", "gref", "gref_umd", "coco"]:
            # h, w = img_size["height"], img_size["width"]
    ...
    def __getitem__(self, idx):
        ...
        if self.testmode:  # default False
            ...
            if self.enable_ptp_aux:
                return img, np.array(img_mask), np.array(text, dtype=int), np.array(text_mask, dtype=int), np.array(bbox, dtype=np.float32), img_file, phrase, bbox_ori, np.array(obj_mask, dtype=int), grid_id
            else:
