...
class OneRef_Dataset_with_MIM(data.Dataset):
    SUPPORTED_DATASETS = {
    ...
    def __init__(self, args, data_root, split_root='data', dataset='referit',
        ...
        self.enable_ref_mim = args.enable_ref_mim
        
        # PTP辅助监督参数
        self.enable_ptp_aux = getattr(args, 'enable_ptp_aux', False)
        self.ptp_grid_size = getattr(args, 'ptp_grid_size', 16)
    ...
    def pull_item(self, idx):
        if self.dataset == 'flickr':  # flickr
            ...
            bbox_xywh = bbox.copy()
            text_sts_score = []
        elif self.dataset == 'mixup_with_sts':
            # img_file, _, bbox, phrase, attri = self.images[idx]  # original data segmentation file format
            # Used for segmentation and alignment
            img_file, img_size, bbox, phrase, obj_mask, text_sts_score = self.images[idx]
            bbox_xywh = bbox.copy()
            if isinstance(img_size, dict):
                image_size = [img_size["height"], img_size["width"]]
        else:  # default
            img_file, img_size, bbox, phrase, obj_mask = self.images[idx]
            bbox_xywh = bbox.copy()
            text_sts_score = []
            if isinstance(img_size, dict):
                image_size = [img_size["height"], img_size["width"]]
        ...
        bbox_ori = bbox.copy()
        """ 
          For the refcoco dataset, uniformly convert the bounding boxes from the xywh (horizontal x, vertical y) 
          format to the x1y1x2y2 format. Then, it is transformed back to xywh at the normalization and pad in Transform.
          The default format of the pth package: unc, unc+, unc-g: xywh; referit, flickr:x1y1x2y2
          comment by xiaolinhui.
        """
        if not (self.dataset == 'referit' or self.dataset == 'flickr'):  # in unc/+/g, from xywh --> x1y1x2y2
            bbox = np.array(bbox, dtype=int)
            bbox[2], bbox[3] = bbox[0] + bbox[2], bbox[1] + bbox[3]
        else:  # in referit, flickr, store the right xywh box
            bbox = np.array(bbox, dtype=int)
            bbox_xywh[2], bbox_xywh[3] = bbox_xywh[2] - bbox_xywh[0], bbox_xywh[3] - bbox_xywh[1]

        """ There are differences in data loading for pre-training with mixed datasets. """
        if self.dataset == "mixup" or self.dataset == "mixup_with_sts":
        ...
        if dataset in ["unc", "unc+", "gref", "gref_umd", "coco"]:
            ...
            if self.enable_seg_mask:
                ...
                obj_mask = [bbox_xywh]  # bbox format: xywh
                bool_obj_mask = convert_coco_poly_mask(np.array([obj_mask]), h, w)  # torch.Size([1, 480, 640])
        else:
            h, w = image_size[0], image_size[1]
            obj_mask = [list(map(float, bbox_xywh))]  # Convert the integer data of bbox to floating-point type.
            bool_obj_mask = convert_coco_poly_mask(np.array([obj_mask]), h, w)  # torch.Size([1, 480, 640])

        return img_file, img, phrase, bbox, bbox_ori, bool_obj_mask, text_sts_score
    ...
    def __getitem__(self, idx):
        ...
        """
         At this point, the processing is carried out one by one. The following are all one-dimensional tensors
         without batch_size.
        """

        img_file, img, phrase, bbox, bbox_ori, obj_mask, text_sts_score = self.pull_item(idx)
        phrase = phrase.lower()
        input_dict = {'img': img, 'box': bbox, 'text': phrase, 'obj_mask': obj_mask}

        if self.enable_ref_mim:
            input_dict, for_visual_tokens, mim_mask_pos, mim_vts_labels = self.transform(input_dict)
            mim_img = for_visual_tokens['img']
        else:
            input_dict = self.transform(input_dict)
            mim_img, mim_mask_pos, mim_vts_labels = None, None, None
        ...
        # 计算Grid ID（用于PTP辅助监督）
        grid_id = None
        if self.enable_ptp_aux:
            from utils.grid_utils import compute_grid_id
            # bbox此时已经是归一化的xywh格式（经过transform处理）
            grid_id = compute_grid_id(bbox, grid_size=self.ptp_grid_size)
            ...
            if self.enable_ref_mim:
                if self.enable_ptp_aux:
                    return img, np.array(img_mask), np.array(text, dtype=int), np.array(text_mask, dtype=int), \
                           ...
                           mim_img, mim_mask_pos, mim_vts_labels, text_sts_score, grid_id
                else:
                    return img, np.array(img_mask), np.array(text, dtype=int), np.array(text_mask, dtype=int), \
                           np.array(bbox, dtype=np.float32), img_file, phrase, bbox_ori, np.array(obj_mask, dtype=int), \
                           mim_img, mim_mask_pos, mim_vts_labels, text_sts_score
            else:
                if self.enable_ptp_aux:
                    return img, np.array(img_mask), np.array(text, dtype=int), np.array(text_mask, dtype=int), \
                           np.array(bbox, dtype=np.float32), img_file, phrase, bbox_ori, np.array(obj_mask, dtype=int), grid_id
                else:
                    return img, np.array(img_mask), np.array(text, dtype=int), np.array(text_mask, dtype=int), \
                           np.array(bbox, dtype=np.float32), img_file, phrase, bbox_ori, np.array(obj_mask, dtype=int)
