...
def train_one_epoch(args, model: torch.nn.Module, data_loader: Iterable,
    ...
    for data_iter_step, batch in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        # model forward
        pred_box, contrastive_loss, visu_sim, seg_mask, mlm_loss, mlm_acc, mlm_sts_pred, mim_pred, mim_vts_pred, ptp_grid_logits = \
...
def validate(args, model: torch.nn.Module, data_loader: Iterable, device: torch.device):
    ...
    for batch in metric_logger.log_every(data_loader, 10, header):
        ...
        # 模型推理时返回5个值：pred_box, seg_mask, vision_cls, language_cls, ptp_grid_logits(None)
        pred_box, seg_mask, img_cls, text_cls, _ = model(img_data.tensors, img_data.mask, text_data)
...
def evaluate_hivg(args, model: torch.nn.Module, data_loader: Iterable, device: torch.device):
    ...
    for _, batch in enumerate(tqdm(data_loader)):
        ...
        # 注意：evaluate_hivg使用的模型接口可能不同，需要兼容处理
        model_output = model(img_data, text_data)
        if len(model_output) == 6:
            # 如果启用PTP，返回6个值
            output, _, _, token_sim, seg_mask, _ = model_output
        else:
            # 旧版本返回5个值
            output, _, _, token_sim, seg_mask = model_output
...
def evaluate_clip_vg(args, model: torch.nn.Module, data_loader: Iterable, device: torch.device):
    ...
    for _, batch in enumerate(tqdm(data_loader)):
        ...
        img_data = img_data.to(device)
        target = target.to(device)
        # 注意：evaluate_clip_vg使用的模型接口可能不同，需要兼容处理
        model_output = model(img_data, text_data)
        if len(model_output) == 6:
            # 如果启用PTP，返回6个值
            output, _, _, _, seg_mask, _ = model_output
        else:
            # 旧版本返回5个值
            output, _, _, _, seg_mask = model_output
