...
class SyntaxGuidedMultiheadAttention(nn.Module):
    ...
    def forward(self, query, key, value, key_padding_mask=None, attn_mask=None, 
        ...
        if self.enable_syntax_bias and text_inputs is not None:
            # 检查text_inputs是否是字符串列表（句法模块需要的格式）
            if isinstance(text_inputs, (list, tuple)) and len(text_inputs) > 0 and isinstance(text_inputs[0], str):
                # 计算句法偏置
                ...
                # 调试：如果text_inputs不是字符串列表格式
                import warnings
                warnings.warn(f"Syntax-guided attention is enabled but text_inputs is not a string list! "
                            f"Type: {type(text_inputs)}, Falling back to standard attention.")
        elif self.enable_syntax_bias and text_inputs is None:
            # 调试：如果启用句法模块但text_inputs为None
            import warnings
            warnings.warn("Syntax-guided attention is enabled but text_inputs is None! Falling back to standard attention.")
