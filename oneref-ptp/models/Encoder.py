...
class EncoderLayer(nn.Module):
    def __init__(self, args, depth, is_moe_layer=False, is_encoder_decoder=False):
        ...
        # TODO: 最核心的关键改动，整个模块命名依然是 MultiheadAttention(), 但是全部 QKV 操作变为 MultiwayNetwork()
...
class Encoder(nn.Module):
    def __init__(
        ...
        output_projection=None,  # 初始化时默认为 None
        ...
        **kwargs
        ...
        if args.encoder_normalize_before and args.normalize_output:  # 默认都是 True，走的此分支
    ...
    def forward_embedding(
        ...
        # 前向编码，src_tokens 为 None, token_embeddings 为传入的数据, positions 在外层已经设计好
        # 先给token_embedding生成一个值
        if token_embedding is None:
            token_embedding = self.embed_tokens(src_tokens)
        x = embed = self.embed_scale * token_embedding
        # 在初始化时已经设计好，self.embed_positions 存在，是 MutliwayEmbedding
    ...
    def forward(
        ...
        **kwargs
        ...
        # encoder_padding_mask 只有编码文本时非空
        if encoder_padding_mask is None:  # 仅图片
            if src_tokens is not None:
        ...
        if multiway_split_position is not None:  # multiway_split_position 仅为图像时为0，本句为含有文本情况下
            assert self.args.multiway
            self.apply(set_split_position(multiway_split_position))

        # 前向位置编码，src_tokens 为 None, token_embeddings 为传入的数据, positions 在外层已经设计好
        ...
        rel_pos_bias = None
        if self.relative_position is not None:  # 相对位置编码，正常为 None，不用管
