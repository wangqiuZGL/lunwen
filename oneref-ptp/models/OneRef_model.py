...
try:
    ...
    try:
        ...
        print("Error: spaCy model not found. Please install with: python -m spacy download en_core_web_md")
        raise
...
class BEiT3ForGrounding(BEiT3Wrapper):
    def __init__(self, sys_args, args, **kwargs):
        ...
        # 保存句法开关以便前向检查
        self.enable_syntax_bias = getattr(sys_args, 'enable_syntax_bias', False)
        ...
        # PTP Auxiliary Head
        self.enable_ptp_aux = getattr(sys_args, 'enable_ptp_aux', False)
        self.ptp_grid_size = getattr(sys_args, 'ptp_grid_size', 16)
        if self.enable_ptp_aux:
            # 使用language_cls作为输入特征
            self.ptp_aux_head = nn.Linear(embed_dim, self.ptp_grid_size * self.ptp_grid_size)
            self.ptp_aux_head.apply(self._init_weights)
            print(f'PTP Auxiliary Head initialized: {embed_dim} -> {self.ptp_grid_size * self.ptp_grid_size} (grid_size={self.ptp_grid_size})')
    ...
    def forward(self, image=None, img_mask=None, text=None, text_len=None, global_step=None, mim_masked_pos=None,
        ...
        # 调试：检查text_inputs的类型（仅在评估时打印一次）
        if not training and self.enable_syntax_bias and text_inputs is not None:
            if not hasattr(self, '_debug_printed'):
                print(f"[DEBUG] Evaluation mode: text_inputs type = {type(text_inputs)}, "
                      f"is_list = {isinstance(text_inputs, (list, tuple))}, "
                      f"len = {len(text_inputs) if isinstance(text_inputs, (list, tuple)) else 'N/A'}, "
                      f"first_elem_type = {type(text_inputs[0]) if isinstance(text_inputs, (list, tuple)) and len(text_inputs) > 0 else 'N/A'}")
        ...
        # Here, uni_mask used in the captioning task is not employed, and no attn_mask is passed in.
        if training and enable_ref_mlm:
        ...
        if training:
            ...
            # PTP辅助任务预测
            if self.enable_ptp_aux:
                # 使用language_cls作为输入（已经计算好的）
                ptp_grid_logits = self.ptp_aux_head(language_cls)  # (B, grid_size²)

            return pred_box, contrast_loss, visu_token_similarity, seg_mask, mlm_loss, mlm_acc, mlm_sts_pred, mim_pred, mim_vts_pred, ptp_grid_logits

        else:  # only_infer
            return pred_box, seg_mask, vision_cls, language_cls, None  # None表示推理时不需要grid_logits
