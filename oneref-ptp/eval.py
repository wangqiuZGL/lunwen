...
def get_args_parser():
    ...
    # Syntax bias parameters
    parser.add_argument('--enable_syntax_bias', action='store_true', help="Enable syntax-guided attention")
    parser.add_argument('--syntax_bias_lambda', type=float, default=0.1, help="Syntax bias strength lambda")

    # PTP Auxiliary Supervision Args (for loading checkpoints that include PTP head)
    parser.add_argument('--enable_ptp_aux', action='store_true',
                        help="Enable PTP auxiliary supervision", default=False)
    parser.add_argument('--ptp_grid_size', type=int, default=16,
                        help="Grid size for PTP (e.g., 16 means 16x16=256 grids)")
    parser.add_argument('--ptp_loss_weight', type=float, default=0.1,
                        help="Weight for PTP auxiliary loss (only used in training)")
