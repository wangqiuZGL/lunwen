...
# 创建输出目录
mkdir -p ${OUTPUT_DIR}/mixup_pretrain
...
######## 1. Warmup 预训练（RefC Mixup）—— 冻结 backbone
echo "=== 阶段1: Warmup 预训练（冻结 backbone，10 epochs）==="
CUDA_VISIBLE_DEVICES=0,1 python -m torch.distributed.launch \
  ...
  --dataset mixup --use_regress_box --frozen_backbone \
  ...
  --output_dir ${OUTPUT_DIR}/mixup_pretrain/mixup_warmup \
  ${SYNTAX_ARGS}

######## 2. Finetune 预训练（RefC Mixup）—— 解冻部分层，使用句法引导注意力
echo "=== 阶段2: Finetune 预训练（解冻部分层，20 epochs）==="
CUDA_VISIBLE_DEVICES=0,1 python -m torch.distributed.launch \
  ...
  --sentencepiece_model ${BEIT3_CHECKPOINT}/beit3.spm \
  --finetune ${OUTPUT_DIR}/mixup_pretrain/mixup_warmup/best_checkpoint.pth \
  --data_root ${DATA_ROOT} --split_root ${SPLIT_ROOT}/mixup_with_refc \
  --output_dir ${OUTPUT_DIR}/mixup_pretrain/mixup_finetune \
  ${SYNTAX_ARGS}
...
CUDA_VISIBLE_DEVICES=0,1 python -m torch.distributed.launch \
  ...
  --eval_model ${OUTPUT_DIR}/mixup_pretrain/mixup_finetune/best_checkpoint.pth \
  --output_dir ${OUTPUT_DIR}/mixup_pretrain/mixup_eval \
  ${SYNTAX_ARGS}
...
echo "最终模型保存在: ${OUTPUT_DIR}/mixup_pretrain/mixup_finetune/best_checkpoint.pth"
echo "评估结果保存在: ${OUTPUT_DIR}/mixup_pretrain/mixup_eval/"
